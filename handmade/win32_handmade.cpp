...
internal debug_read_file_result DEBUGPlatformReadEntireFile(char *Filename)
{
    ...
    if (FileHandle != INVALID_HANDLE_VALUE)
    {
        ...
        if (GetFileSizeEx(FileHandle, &FileSize))
        {
            ...
            Result.Contents =
                VirtualAlloc(0, Result.ContentsSize, MEM_RESERVE | MEM_COMMIT,
                             PAGE_READWRITE);
            if (Result.Contents)
            {
                DWORD BytesRead;
                                    ...
                                    real32 TargetSecondsPerFrame)
{
    ...
    for (int MarkerIndex = 0; MarkerIndex < MarkerCount; ++MarkerIndex)
    {
        ...
        // ASSERT(ThisMarker->ExpectedFlipPlayCursor <
        // SoundOutput->SecondaryBufferSize);//this is an added value
        ...
        if (MarkerIndex == CurrentMarkerIndex)
        {
            ...
            Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top,
                                       Bottom, ThisMarker->OutputPlayCursor,
                                       PlayColor);
            Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top,
                                       Bottom, ThisMarker->OutputWriteCursor,
                                       WriteColor);
            ...
            Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top,
                                       Bottom, ThisMarker->OutputLocation,
                                       PlayColor);
        ...
        Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top,
                                   ...
                                   PlayColor);
        Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top,
                                   Bottom, ThisMarker->FlipWriteCursor,
                                   WriteColor);
    }
}
                ...
                while (GlobalRunning)
                {
                    ...
                    *NewKeyboardController = {};
                    ...
                    for (int ButtonIndex = 0;
                         ButtonIndex <
                         ARRAY_COUNT(NewKeyboardController->Buttons);
                         ++ButtonIndex)
                    {
                    ...
                    if (!GlobalPause)
                    {
                        ...
                        // TODO: Need to not poll disconnected controllers to
                        // avoid xinput frame rate
                        //  hit on older libraries
                        // TODO: should we poll this more fdrequently?
                        DWORD MaxControllerCount = XUSER_MAX_COUNT;
                        if (MaxControllerCount >
                            (ARRAY_COUNT(NewInput->Controllers) - 1))
                        {
                            MaxControllerCount =
                                (ARRAY_COUNT(NewInput->Controllers) - 1);
                        }
                             ...
                             ++ControllerIndex)
                        {
                                ...
                                ERROR_SUCCESS)
                            {
                                ...
                                // TODO: see if controlerState.dwPacketNumber
                        ...
                        real32 FromBeginToAudioSeconds = Win32GetSecondsElapsed(
                                ...
                                &PlayCursor, &WriteCursor) == DS_OK)
                        {
                            ...
                            samples we think out game update loop may vary by
                            (let's say up to 2ms).

                            When we wake up to write audio, we will look and see
                            what the play cursor position is and we will
                            forecast ahead where we whink the play cursor will
                            ...
                            audio plus the safety margin's worth of guard
                            samples.
                            */
                            if (!SoundIsValid)
                            {
                            ...
                            win32_debug_time_marker *Marker =
                                &DebugTimeMarkers[DebugTimeMarkersIndex];
                            ...
                            // Samplespersecond already account for the two
                            // channels
                        ...
                        if (SecondsElapsedForFrame < TargetSecondsPerFrame)
                        {
                            ...
                            real32 TestSecondsElapsedForFrame =
                                Win32GetSecondsElapsed(LastCounter,
                                                       Win32GetWallClock());
                            if (TestSecondsElapsedForFrame <
                                TargetSecondsPerFrame)
                            {
                                // TODO: Log Missed sleep here!
                            }

                            while (SecondsElapsedForFrame <
                                   TargetSecondsPerFrame)
                            {
                        ...
                        Win32DebugSyncDisplay(
                            &GlobalBackbuffer, ARRAY_COUNT(DebugTimeMarkers),
                            DebugTimeMarkers, DebugTimeMarkersIndex - 1,
                            &SoundOutput, TargetSecondsPerFrame);
#endif
                        Win32DisplayBufferInWindow(
                            GlobalBackbuffer, DeviceContext, Dimension.Width,
                            Dimension.Height);
                        ...
                        sprintf_s(Msg,
                                  "%.02fms/f, %.02ff/s, (%.02fws/f) whiles=%d "
                                  "sleepms=%d, Slept=%.02fms\n",
                                  MSPerFrame, FPS, WorkSecondsElapsed * 1000.0f,
                                  whiles, SleepMS, Slept * 1000.0f);
                        OutputDebugStringA(Msg);

#if HANDMADE_INTERNAL
                        ++DebugTimeMarkersIndex;
                        if (DebugTimeMarkersIndex ==
                            ARRAY_COUNT(DebugTimeMarkers))
                        {
